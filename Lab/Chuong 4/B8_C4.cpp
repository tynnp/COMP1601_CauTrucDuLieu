...
public:
    Ticket();
    Ticket(string customerName, int seatNumber);
...
private:
    LinkedList<int> _A;
    Queue<int> _B;
    LinkedList<Ticket> _C;

public:
    void init(int n);
    void takeQueueNumber();
...
Ticket::Ticket(string customerName, int seatNumber) {
    _strCustomerName = customerName;
    _iSeatNumber = seatNumber;
}

Ticket::Ticket(const Ticket &other) {
    _strCustomerName = other._strCustomerName;
...
void Program::init(int n) {
    for (int i = 1; i <= n; i++)
        _A.addBack(i);
}

void Program::takeQueueNumber() {
    int iLastNumber = _B.empty() ? 0 : _B.back();
    _B.push(iLastNumber + 1);
    cout << "Khach nhan so thu tu: " << iLastNumber + 1 << endl;
}

void Program::buyTicket(Ticket ticket) {
    if (_A.empty()) {
        cout << "Het ghe trong!" << endl;
        return;
    }

    if (_B.empty()) {
        cout << "Khong co khach trong hang cho!" << endl;
        return;
    }

    int seatIndex = _A.find(ticket.getSeatNumber());
    if (seatIndex == -1) {
        cout << "Ghe khong hop le hoac da duoc dat!" << endl;
        return;
    }

    _A.removeAt(seatIndex);
    int iQueueNumber = _B.front();
    _B.pop();
    _C.addBack(ticket);

    cout << "Khach: " << ticket.getCustomerName() << " voi so thu tu " << iQueueNumber << " da mua ve ghe so " << ticket.getSeatNumber() << "." << endl;
}

void Program::cancelTicket(int seatNumber) {
    for (int i = 0; i < _C.size(); i++) {
        if (_C.getAt(i).getSeatNumber() == seatNumber) {
            _A.addBack(seatNumber);
            _C.removeAt(i);
            cout << "Da huy ve ghe so " << seatNumber << "." << endl;
            return;
        }
    }
}

void Program::displaySoldTickets() {
    cout << "Danh sach ve da ban: ";
    for (int i = 0; i < _C.size(); i++)
        cout << "Ghe so " << _C.getAt(i).getSeatNumber() << ", Khach hang: " << _C.getAt(i).getCustomerName() << endl;
}
...
void Program::menu() {
    ...
    do {
        cout << "\n==================== MENU ====================\n";
        cout << "1. Khoi tao danh sach ghe trong\n";
        cout << "2. Lay so thu tu cho khach hang\n";
        cout << "3. Ban ve cho khach hang\n";
        cout << "4. Huy ve\n";
        cout << "5. Hien thi danh sach ve da ban\n";
        ...
        case 1: {
            ...
            cin >> n;
            init(n);
            cout << "Khoi tao danh sach " << n << " ghe trong thanh cong!" << endl;
            break;
        }
        case 2: {
            takeQueueNumber();
        ...
        case 3: {
            ...
            cout << "Nhap so ghe muon mua: ";
            cin >> seatNumber;
            Ticket ticket(customerName, seatNumber);
            buyTicket(ticket);
            break;
        }
        case 4: {
            int seatNumber;
            cout << "Nhap so ghe muon huy: ";
            cin >> seatNumber;
